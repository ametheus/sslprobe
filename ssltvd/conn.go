...
func (hc *halfConn) incSeq() {
	...
	panic("ssltvd: sequence number wraparound")
}
...
func (e RecordHeaderError) Error() string { return "ssltvd: " + e.Msg }
...
func (c *Conn) readRecord(want recordType) error {
	...
	default:
		...
		return c.in.setErrorLocked(errors.New("ssltvd: unknown record type requested"))
	case recordTypeHandshake, recordTypeChangeCipherSpec:
		if c.handshakeComplete {
			c.sendAlert(alertInternalError)
			return c.in.setErrorLocked(errors.New("ssltvd: handshake or ChangeCipherSpec requested after handshake complete"))
		}
...
var errClosed = errors.New("ssltvd: use of closed connection")
...
func (c *Conn) VerifyHostname(host string) error {
	...
	defer c.handshakeMutex.Unlock()
	if !c.isClient {
		return errors.New("ssltvd: VerifyHostname called on TLS server connection")
	}
	if !c.handshakeComplete {
		return errors.New("ssltvd: handshake has not yet been performed")
	}
	if len(c.verifiedChains) == 0 {
		return errors.New("ssltvd: handshake did not verify certificate chain")
	}

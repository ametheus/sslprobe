...
var errClientKeyExchange = errors.New("ssltvd: invalid ClientKeyExchange message")
var errServerKeyExchange = errors.New("ssltvd: invalid ServerKeyExchange message")
...
func (ka rsaKeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("ssltvd: certificate private key does not implement crypto.Decrypter")
	}
...
func (ka rsaKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	return errors.New("ssltvd: unexpected ServerKeyExchange")
}
...
func hashForServerKeyExchange(sigAndHash signatureAndHash, version uint16, slices ...[]byte) ([]byte, crypto.Hash, error) {
	if version >= VersionTLS12 {
		if !isSupportedSignatureAndHash(sigAndHash, supportedSignatureAlgorithms) {
			return nil, crypto.Hash(0), errors.New("ssltvd: unsupported hash function used by peer")
		}
...
func pickTLS12HashForSignature(sigType uint8, clientList []signatureAndHash) (uint8, error) {
	...
	return 0, errors.New("ssltvd: client doesn't support any common hash functions")
}
...
NextCandidate:
	...
	if curveid == 0 {
		return nil, errors.New("ssltvd: no supported elliptic curves offered")
	}

	var ok bool
	if ka.curve, ok = curveForCurveID(curveid); !ok {
		return nil, errors.New("ssltvd: preferredCurves includes unsupported curve")
	}
	...
	if !ok {
		return nil, errors.New("ssltvd: certificate private key does not implement crypto.Signer")
	}
	var sig []byte
	switch ka.sigType {
	case signatureECDSA:
...
func (ka *ecdheKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	if skx.key[0] != 3 { // named curve
		return errors.New("ssltvd: server selected unsupported curve")
	}
